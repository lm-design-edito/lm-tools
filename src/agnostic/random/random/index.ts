 ...
 *
 * @param [bound1=1] - The lower bound if `bound2` is provided, otherwise the upper bound (min is 0).
 ...
 * @param [bound1=1] - The lower bound if `bound2` is provided, otherwise the upper bound (min is 0).
 * @param [bound2] - Optional upper bound. If omitted, the range is `[0, bound1)`.
 * @returns A random integer in `[min, max)`, or `undefined` if bounds are invalid.
 */
