...
export namespace Subpaths {
  ...
  export type ChildDetails = {
    /** The type of the child entry. */
    type: ChildType
    /** Whether the child entry is hidden (starts with '.'). */
    hidden: boolean
    /** The real path of the child entry (resolved if it's a symlink). */
    realPath: string
  }
  ...
  export type ListContext = {
    ...
    depth?: number
    /** Cached lstat result for the current path. */
    lstats?: Stats | null
    /** Root path from which the listing started. */
  ...
  export type ListOptions = {
    ...
    files?: boolean
    /** Whether to include symlinks in the results. @default true */
    symlinks?: boolean
    /** Whether to include hidden files/directories (starting with '.'). @default true */
    hidden?: boolean
    /** Whether to follow symlinks and include their contents. @default false */
    followSimlinks?: boolean
    /** Whether to deduplicate paths when following symlinks. @default false */
    dedupeSimlinksContents?: boolean
    /** Maximum recursion depth. @default Infinity */
    maxDepth?: number
    /** Whether to return relative paths instead of absolute paths. @default false */
    returnRelative?: boolean
    /** Patterns to exclude from the results. */
    exclude?: RegExp | string | (RegExp | string)[] | null
    /** Patterns to include in the results (takes precedence over exclude). */
    include?: RegExp | string | (RegExp | string)[] | null
    /** Custom filter function to further refine results. */
